...
class LIS3DH:
    ...
    def __init__(self, intPin):
        ...
        # Latch interrupt for INT
        self._write_register_byte(REG_CTRL5, 0x08)

        # Initialise interrupt pin (if supplied)
        if intPin:
    ...
    def data_rate(self):
        """
        The data rate of the accelerometer.  Can be DATA_RATE_400_HZ, DATA_RATE_200_HZ, DATA_RATE_100_HZ,
        DATA_RATE_50_HZ, DATA_RATE_25_HZ, DATA_RATE_10_HZ, DATA_RATE_1_HZ, DATA_RATE_POWERDOWN,
        DATA_RATE_LOWPOWER_1K6HZ, or DATA_RATE_LOWPOWER_5KHZ
        """
    ...
    def get_acceleration(self, as_mps2 = True):
        ...
        param: bool as_mps2: flag to either return as m / s ^ 2 or as Gs, default is as m / s ^ 2 (True)
        """
        g = STANDARD_GRAVITY if as_mps2 else 1.0  # Use to do any Gs to m / s ^ 2 conversion
        divider = 1
        ...
        # Convert from Gs to m / s ^ 2 (if requested) and adjust for the range
        x = (x / divider) * g
        y = (y / divider) * g
        z = (z / divider) * g
    ...
    def read_adc_mV(self, adc):
        """
        Read the specified analog to digital converter value in millivolts, ADC must be a value 1,
        2, or 3,  NOTE the ADC can only measure voltages in the range of ~900-1200mV!
        """
    ...
    def tapped(self):
        """
        True if a tap was detected recently, whether its a single tap or double tap is determined by the
        tap param on set_tap(), tapped may be True over multiple reads even if only a single tap or
        single double tap occurred if the interrupt (int) pin is not specified
        ...
        The following example uses ``i2c`` and specifies the interrupt pin:
            ...
            i2c = machine.I2C(Pin(22), Pin(23))  # I2C on pins 22 and 23
            lis3dh = lis3dh.LIS3DH_I2C(i2c, intPin = 21)  # Interrupt on pin 21
            lis3dh.range = lis3dh.RANGE_8_G
    ...
    def set_tap(self, tap, threshold, *, time_limit = 10, time_latency = 20, time_window = 255,
                click_cfg = None):
        """
        Set the tap detection parameters
            ...
            :param int tap: 0 to disable tap detection, 1 to detect only single taps, and 2 to detect only
            double taps
            :param int threshold: A threshold for the tap detection, the higher the value the less sensitive
            the detection, this changes based on the accelerometer range, good values are 5-10 for 16G, 
    ...
    def _read_register(self, register, length):
        """
        Read an arbitrarily long register (specified by length number of bytes) and return a bytearray
        of the retrieved data, Subclasses MUST implement this!
        """
        raise NotImplementedError

    def _write_register_byte(self, register, value):
        """
        Write a single byte register at the specified register address, Subclasses MUST implement
        this!
        """
        raise NotImplementedError
