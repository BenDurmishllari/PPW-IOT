...
class VL53L0X:
 def __init__(self,i2c,address=I2C_ADDRESS):
  ...
  self.init()
  self._started=False
 def _registers(self,register,values=None,buf='B'):
  if values is None:
   size=struct.calcsize(buf)
   data=self.i2c.readfrom_mem(self.address,register,size)
   values=struct.unpack(buf,data)
   return values
  data=struct.pack(buf,*values)
  self.i2c.writeto_mem(self.address,register,data)
 def _register(self,register,value=None,buf='B'):
  if value is None:
   return self._registers(register,buf=buf)[0]
  self._registers(register,(value,),buf=buf)
 ...
 def init(self,power2v8=True):
  ...
  self._register(_FINAL_RATE_RTN_LIMIT,int(0.25*(1<<7)),buf='>H')
  self._register(_SYSTEM_SEQUENCE,0xff)
  spad_count,is_aperture=self._spad_info()
  spad_map=bytearray(self._registers(_SPAD_ENABLES,buf='6B'))
  self._config((0xff,0x01),(_SPAD_REF_START,0x00),(_SPAD_NUM_REQUESTED,0x2c),(0xff,0x00),(_REF_EN_START_SELECT,0xb4),)
  spads_enabled=0
  for i in range(48):
 ...
 def start(self,period=0):
  ...
  if period:
   oscilator=self._register(_OSC_CALIBRATE,buf='>H')
   if oscilator:
    period*=oscilator
   self._register(_MEASURE_PERIOD,period,buf='>H')
 ...
 def read(self):
  if not self._started:
   self._config((0x80,0x01),(0xFF,0x01),(0x00,0x00),(0x91,self._stop_variable),(0x00,0x01),(0xFF,0x00),(0x80,0x00),(_SYSRANGE_START,0x01),)
   for timeout in range(_IO_TIMEOUT):
  ...
  for timeout in range(_IO_TIMEOUT):
   ...
   raise TimeoutError()
  value=self._register(_RESULT_RANGE_STATUS+10,buf='>H')
  self._register(_INTERRUPT_CLEAR,0x01)
  return value
