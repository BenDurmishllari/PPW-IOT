...
class _BaseRTC:
 ...
 DateTimeTuple=ucollections.namedtuple("DateTimeTuple",["year","month","day","weekday","hour","minute","second","millisecond"])
 ...
 def datetime(self,datetime=None):
  if datetime is None:
   ...
   if self._SWAP_DAY_WEEKDAY:
    ...
    day=buffer[4]
    weekday=buffer[3]
   return self.datetime_tuple(year=self._bcd2bin(buffer[6])+2000,month=self._bcd2bin(buffer[5]),day=self._bcd2bin(day),weekday=self._bcd2bin(weekday),hour=self._bcd2bin(buffer[2]),minute=self._bcd2bin(buffer[1]),second=self._bcd2bin(buffer[0]),)
 ...
 def datetime_tuple(self,year=None,month=None,day=None,weekday=None,hour=None,minute=None,second=None,millisecond=None):
  return self.DateTimeTuple(year,month,day,weekday,hour,minute,second,millisecond)
 def tuple2seconds(self,datetime):
  return utime.mktime((datetime.year,datetime.month,datetime.day,datetime.hour,datetime.minute,datetime.second,datetime.weekday,0))
...
class PCF8523(_BaseRTC):
 ...
 def alarm_time(self,datetime=None):
  if datetime is None:
   buffer=self.i2c.readfrom_mem(self.address,self._ALARM_REGISTER,4)
   return self.datetime_tuple(weekday=self._bcd2bin(buffer[3]&0x7f)if not buffer[3]&0x80 else None,day=self._bcd2bin(buffer[2]&0x7f)if not buffer[2]&0x80 else None,hour=self._bcd2bin(buffer[1]&0x7f)if not buffer[1]&0x80 else None,minute=self._bcd2bin(buffer[0]&0x7f)if not buffer[0]&0x80 else None,)
  datetime=self.datetime_tuple(*datetime)
  buffer=bytearray(4)
  buffer[0]=(self._bin2bcd(datetime.minute)if datetime.minute is not None else 0x80)
  buffer[1]=(self._bin2bcd(datetime.hour)if datetime.hour is not None else 0x80)
  buffer[2]=(self._bin2bcd(datetime.day)if datetime.day is not None else 0x80)
  buffer[3]=(self._bin2bcd(datetime.weekday)|0b01000000 if datetime.weekday is not None else 0x80)
